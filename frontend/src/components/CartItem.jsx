  ...
  return (
    ...
    >
      <div className="relative w-full md:w-48 h-48 lg:w-56 lg:h-56 rounded-[32px] overflow-hidden bg-slate-100 shrink-0">
        <img src={item.image_url || 'https://via.placeholder.com/400x300'} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
      ...
      <div className="flex-1 w-full space-y-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          ...
          <div className="text-3xl font-black text-slate-950">${(Number(item.price) * item.quantity).toFixed(2)}</div>
        </div>
